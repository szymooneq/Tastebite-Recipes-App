...
function FileField({ name, label, error, imgValue, value, onChange }: IFileField): JSX.Element {
	...
	const labelColors = error ? 'text-red-700 dark:text-red-500' : 'text-gray-900 dark:text-gray-300'
	...
	return (
		<div className="mb-4">
			<label htmlFor={name} className={`block mb-2 text-sm font-medium ${labelColors}`}>
				{label}
			</label>
			...
			<div className="inline-flex w-full justify-between">
				...
				{preview && (
					<Button onClick={() => handleRemoveImage()} color="red" type="button" disabled={false}>
						Usuń
					</Button>
				)}
			</div>
