const InputText = props => {
  return (
    <div className="mx-auto mb-4 w-96">
      <label 
        htmlFor={props.id} 
        className={`block mb-2 text-sm font-medium ${props.error ? 'text-red-700 dark:text-red-500' : 'text-gray-900 dark:text-gray-300'} `}>{props.label}</label>
      ...
      <input
        ...
        className={`block w-full p-2.5 border text-sm rounded-lg outline-none ${props.error 
          ? 'bg-red-50 border-red-500 text-red-900 placeholder-red-700 focus:ring-red-500 dark:bg-gray-700 focus:border-red-500 dark:text-red-500 dark:placeholder-red-500 dark:border-red-500' : 'bg-gray-50 border-gray-300 text-gray-900  focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500'}`} />

        {props.error ? <p className="mt-2 text-sm font-semibold text-red-600 dark:text-red-500">{props.error}</p> : null}
    </div>
  )
}
...
const InputSelect = props => {
  return (
    <div className="mx-auto mb-4 w-96">
      <label htmlFor={props.id} className="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-400">{props.label}</label>
      <select 
        id={props.id}
        value={props.value} 
        onChange={e => props.onChange(e.target.value)}
      ...
      {props.options.map(option => (
        <div key={option.value} className="flex items-center mb-4">
          ...
          <label 
            ...
            className="ml-2 text-sm font-medium text-gray-900 dark:text-gray-300">{option.label}</label>
        </div>
      ))}
    </div>
  )
  ...
  return (
    <div className="mx-auto mb-4 w-96">
      <label htmlFor={props.id} className="inline-flex relative items-center cursor-pointer">
        ...
        <span className="ml-3 text-sm font-medium text-gray-900 dark:text-gray-300">{props.value ? 'Aktywny' : 'Ukryty'}</span>
      </label>
    </div>
  )
}
