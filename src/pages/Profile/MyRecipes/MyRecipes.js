...
export default function MyRecipes() {
  ...
  const deleteHandler = async product => {
    try {
      ...
      await deleteDoc(doc(db, "recipes", product.id))
      fetchData()
    } catch (error) {
      console.log(error)
    }
  ...
  return loading ? <LoadingIcon /> : (
    <>
        ...
        <table className="mx-auto w-full text-sm text-gray-500 dark:text-gray-400">
          ...
          <tbody>
            {recipes.map((product, id) => (
              <tr key={id} className="bg-white hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-600">
                ...
                <td className="p-4">
                  {product.status 
                    ? <span className="mr-2 px-2.5 py-0.5 text-xs font-semibold rounded-full text-green-800 bg-green-100 dark:bg-green-200 dark:text-green-900">Aktywny</span>
                    : <span className="mr-2 px-2.5 py-0.5 text-xs font-semibold rounded-full text-red-800 bg-red-100 dark:bg-red-200 dark:text-red-900">Ukryty</span>
                  }
                </td>
      ...
      <Link 
        to={"dodaj"} 
        className="mx-auto mt-4 p-2.5 block w-max text-sm font-bold rounded-lg focus:ring-4 focus:outline-none text-white bg-green-700 dark:bg-green-600 hover:bg-green-800 dark:hover:bg-green-700 focus:ring-green-200 dark:focus:ring-green-800">Nowy przepis</Link>
    </>
  )
}